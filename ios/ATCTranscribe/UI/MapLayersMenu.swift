...
/// master switch. All state (`chartLayer` / `showAirspace` / `showNearby` / `showWxRadar` /
/// `mapBackgroundEnabled`) is persisted on `AppModel`, so this menu and the always-on home map stay
/// in sync.
