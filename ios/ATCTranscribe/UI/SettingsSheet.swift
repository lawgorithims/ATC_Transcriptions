...
struct SettingsSheet: View {
        ...
        return NavigationLink { destination() } label: {
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    Text(subtitle).font(.dsLabelS).foregroundStyle(p.textDim)
                }
                Spacer()
    ...
    @ViewBuilder private var developerCategory: some View {
        ...
        Card(title: "Map projection") {
            VStack(alignment: .leading, spacing: 10) {
                Toggle(isOn: $model.useGlobeProjection) {
                    Text("Globe projection (experimental)").font(.dsLabel).foregroundStyle(p.text)
                }
                .tint(p.accent)
        ...
        #else
        Card(title: "Map projection") {
            Text("MapLibre is not linked in this build.").font(.dsLabelS).foregroundStyle(p.textDim)
        }
        #endif
        ...
        Card(title: "Transcript log") {
            VStack(alignment: .leading, spacing: 10) {
                Toggle(isOn: $model.transcriptLoggingEnabled) {
                    Text("Save transcript log").font(.dsLabel).foregroundStyle(p.text)
                }
        ...
        Card(title: "Transcript correction") {
            VStack(alignment: .leading, spacing: 12) {
                Toggle(isOn: $model.correctionEnabled) {
                    Text("Vocabulary correction").font(.dsLabel).foregroundStyle(p.text)
                }
                ...
                Text("AI context fixer").font(.dsLabel).foregroundStyle(p.text)
                HStack(spacing: 8) {
                ...
                Toggle(isOn: $model.skipWhenConfident) {
                    Text("Skip the AI fixer when confident").font(.dsLabel).foregroundStyle(p.text)
                }
                ...
                Rectangle().fill(p.border).frame(height: 1)
                Text("Cloud fixer (advanced)").font(.dsLabel).foregroundStyle(p.text)
        ...
        Card(title: "Speakers") {
            VStack(alignment: .leading, spacing: 10) {
                Toggle(isOn: $model.diarizationEnabled) {
                    Text("Separate speakers").font(.dsLabel).foregroundStyle(p.text)
                }
                ...
                Toggle(isOn: $model.acousticFillEnabled) {
                    Text("Guess speaker by voice (experimental)").font(.dsLabel).foregroundStyle(p.text)
                }
        ...
        Card(title: "Live traffic") {
            VStack(alignment: .leading, spacing: 10) {
                Toggle(isOn: $model.adsbStreamingEnabled) {
                    Text("Online ADS-B streaming").font(.dsLabel).foregroundStyle(p.text)
                }
                ...
                Text(ADSBService.attribution)
                    .font(.dsLabelS).foregroundStyle(p.textDim.opacity(0.8))
            }
        }
        Card(title: "Stratux receiver") {
            VStack(alignment: .leading, spacing: 10) {
                Toggle(isOn: $model.stratuxEnabled) {
                    Text("Stratux link").font(.dsLabel).foregroundStyle(p.text)
                }
                ...
                Text("Connect to a Stratux ADS-B/GPS receiver over its Wi-Fi for cockpit audio plus on-board traffic and GPS — no internet needed in flight. Turn the Stratux link on (here or in the Stratux bar on the console) to stream traffic + GPS; pick “Stratux receiver” as the input source to transcribe its audio.")
                    .font(.dsLabelS).foregroundStyle(p.textDim)
                HStack(spacing: 10) {
                    Text("Address").font(.dsLabel).foregroundStyle(p.textDim)
                ...
                HStack(spacing: 10) {
                    Text("Audio port").font(.dsLabel).foregroundStyle(p.textDim)
                        .frame(width: 84, alignment: .leading)
                    TextField("8090", value: $model.stratuxAudioPort, format: .number.grouping(.never))
                        .textFieldStyle(.plain).keyboardType(.numberPad).font(.dsLabel)
                        .padding(.horizontal, 10).padding(.vertical, 8)
                        .background(p.surfaceAlt).clipShape(RoundedRectangle(cornerRadius: DS.Radius.r2))
                        .overlay(RoundedRectangle(cornerRadius: DS.Radius.r2).stroke(p.border, lineWidth: 1))
                        .frame(maxWidth: 120)
                ...
                Text("Traffic + GPS use the receiver's web API on \(model.stratuxHost); cockpit audio streams from the sidecar on port \(String(model.stratuxAudioPort)) at /audio.raw. Set the sidecar up with the guide in Tools/stratux. iOS will ask once for permission to find devices on your local network.")
                    .font(.dsLabelS).foregroundStyle(p.textDim)
                Text(StratuxService.attribution).font(.dsLabelS).foregroundStyle(p.textDim.opacity(0.8))
            }
        }
        Card(title: "ForeFlight") {
            VStack(alignment: .leading, spacing: 10) {
                Toggle(isOn: $model.foreflightEnabled) {
                    Text("ForeFlight hand-off").font(.dsLabel).foregroundStyle(p.text)
                }
        ...
        Card(title: "Display") {
            VStack(alignment: .leading, spacing: 10) {
                Toggle(isOn: $model.showDebug) {
                    Text("Show performance data").font(.dsLabel).foregroundStyle(p.text)
                }
        ...
        Card(title: "Map engine") {
            VStack(alignment: .leading, spacing: 10) {
                Toggle(isOn: $model.useMapLibreMap) {
                    Text("New GPU map (MapLibre)").font(.dsLabel).foregroundStyle(p.text)
                }
